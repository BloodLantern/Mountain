...
struct MultiBootData gMultiBootParamData;
const u8* gDataSentPointer;
u32 gDataSentSize;

u16 gRegIme_Backup;
u16 gRegIe_Backup;
u16 gRegTm3Cnt_H_Backup;
u16 gRegSiocnt_Backup;
u16 gRegRcnt_Backup;
...
u16 gUnk_30058a8;
struct CableLink_3005890 gUnk_3005890;

u32 gErrorFlag;
u16 gUnk_30058c0[4];
CableLinkBuffer1_T gUnk_30058c4;

struct CableLinkInfo gCableLinkInfo;
u16 gUnk_30058d0;
u8 gUnk_30058d2;
u8 gUnk_30058d3;
u8 gUnk_3005b54;
...
void unk_89be4(void)
{
    ...
    switch (gUnk_3005890.unk_1)
    {
        ...
        case 5:
            ...
            for (i = 1, var_0 = 1; i < gUnk_30058b0; i++)
            {
                if (buffer[i] == 1)
                    var_0++;
                else if (buffer[i] == 2)
...
u8* CableLinkCheckError(void)
{
    ...
    *pError = CableLinkDetectError(&gUnk_30058cc, gUnk_30058c0, gUnk_30058c4);
    ...
    if (*pError & CABLE_LINK_ERROR_ID_OVER)
    {
        ...
        CableLinkDrawErrorStr(sErrorString_IdOver, gVramBuffer + 0xE304, 12);
    }

    if (*pError & CABLE_LINK_ERROR_CHECKSUM)
    {
        gIoTransferInfo.errorFlag |= 2;
        CableLinkDrawErrorStr(sErrorString_ChecksumError, gVramBuffer + 0xE344, 12);
    }

    if (*pError & CABLE_LINK_ERROR_HARDWARE)
    {
        gIoTransferInfo.errorFlag |= 4;
        CableLinkDrawErrorStr(sErrorString_HardwareError, gVramBuffer + 0xE384, 12);
    }

    if (*pError & CABLE_LINK_ERROR_SEND_OVERFLOW)
    {
        gIoTransferInfo.errorFlag |= 8;
        CableLinkDrawErrorStr(sErrorString_SendOverflow, gVramBuffer + 0xE3C4, 12);
    }

    if (*pError & CABLE_LINK_ERROR_RECEIVE_OVERFLOW)
    {
        gIoTransferInfo.errorFlag |= 16;
        CableLinkDrawErrorStr(sErrorString_ReceiveOverflow, gVramBuffer + 0xE404, 12);
    }

    if (*pError & CABLE_LINK_ERROR_SIO_INTERNAL)
    {
        gIoTransferInfo.errorFlag |= 32;
        CableLinkDrawErrorStr(sErrorString_SioInternal, gVramBuffer + 0xE444, 12);
    }

    if (*pError & CABLE_LINK_ERROR_SIO_STOP)
    {
        gIoTransferInfo.errorFlag |= 64;
        CableLinkDrawErrorStr(sErrorString_SioStop, gVramBuffer + 0xE484, 12);
    }
