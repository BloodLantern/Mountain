...
u8 gSramOperationStage;
u8 gSramCorruptFlag;
struct Sram gSram;

u8 gUnk_3000c20;
struct SaveFileInfo gSaveFilesInfo[3];
struct StartingInfo gStartingInfo;

// Place holder symbols?
u16 gSramDemoInputData[DEMO_MAX_DURATION];
u16 gSramDemoInputDuration[DEMO_MAX_DURATION];
...
void SramRead_All(void)
{
    ...
    FILE* f;
    fopen_s(&f, "save.bin", "rb");
    if (f)
    {
        fread(gEwramBuffer, sizeof(gEwramBuffer), 1, f);
        fclose(f);
    }
...
void SramTestFlash(void)
{
    ...
    if (SramWriteChecked(sMetZeroSramCheck_Text, gSramBuffer + OFFSET_OF(struct Sram, MetZeroSramCheck_Text), SRAM_TEXT_SIZE))
        flags = 1; // Internal check failed

    // Read the text previously written into a local buffer
    SramWriteUnchecked(gSramBuffer + OFFSET_OF(struct Sram, MetZeroSramCheck_Text), text, SRAM_TEXT_SIZE);
    ...
    if (SramWriteChecked(text, gSramBuffer + OFFSET_OF(struct Sram, MetZeroSramCheck_Text), SRAM_TEXT_SIZE))
...
void SramLoad_DemoRamValues(u8 loadSamusData, u8 demoNumber)
{
    ...
    if (loadSamusData == FALSE)
    {
        ...
        DmaTransfer(3, pDemo->visitedMinimapTiles, gVisitedMinimapTiles + gCurrentArea * MINIMAP_SIZE,
