...
void TransferSamusGraphics(u32 updatePalette, struct SamusPhysics* pPhysics)
{
    ...
    if (pPhysics->torsoGfxSize != 0)
        DMA_SET(3, pPhysics->pTorsoGfx, gVramBuffer + 0x10400, DMA_ENABLE << 16 | pPhysics->torsoGfxSize / 2);

    if (pPhysics->legsGfxSize != 0)
        DMA_SET(3, pPhysics->pLegsGfx, gVramBuffer + 0x10280, DMA_ENABLE << 16 | pPhysics->legsGfxSize / 2);

    if (pPhysics->bodyLowerHalfGfxSize != 0)
        DMA_SET(3, pPhysics->pBodyLowerHalfGfx, gVramBuffer + 0x10680, DMA_ENABLE << 16 | pPhysics->bodyLowerHalfGfxSize / 2);

    if (pPhysics->armCannonGfxUpperSize != 0)
        DMA_SET(3, pPhysics->pArmCannonGfxUpper, gVramBuffer + 0x10800, DMA_ENABLE << 16 | pPhysics->armCannonGfxUpperSize / 2);

    if (pPhysics->armCannonGfxLowerSize != 0)
        DMA_SET(3, pPhysics->pArmCannonGfxLower, gVramBuffer + 0x10C00, DMA_ENABLE << 16 | pPhysics->armCannonGfxLowerSize / 2);

    if (pPhysics->screwSpeedGfxSize != 0)
        DMA_SET(3, pPhysics->pScrewSpeedGfx, gVramBuffer + 0x10840, DMA_ENABLE << 16 | pPhysics->screwSpeedGfxSize / 2);

    if (pPhysics->screwShinesparkGfxSize != 0)
        DMA_SET(3, pPhysics->pScrewShinesparkGfx, gVramBuffer + 0x10C40, DMA_ENABLE << 16 | pPhysics->screwShinesparkGfxSize / 2);

    if (updatePalette)
        DMA_SET(3, gSamusPalette, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), DMA_ENABLE << 16 | gSamusPaletteSize / 2);
}
...
void InitAndLoadGenerics(void)
{
    ...
    if (gPauseScreenFlag != PAUSE_SCREEN_NONE || gCurrentCutscene != 0)
        DmaTransfer(3, gEwramBuffer + 0x1E000, VRAM_OBJ, 0x4000, 0x10);

    gDebugFlag = FALSE;
    DMA_SET(3, sCommonSpritesPAL, gPalramBuffer + 0x240, DMA_ENABLE << 16 | sizeof(sCommonSpritesPAL) / 2);
    SamusInit();
    ...
    {
        DmaTransfer(3, gEwramBuffer + 0x22000, gVramBuffer + 0x14000, 0x4000, 0x10);
        DmaTransfer(3, gEwramBuffer + 0x35700, gPalramBuffer + 0x300, 0x100, 0x10);
        DmaTransfer(3, gEwramBuffer + 0x35460, gPalramBuffer + 0x60, 0x1A0, 0x10);
    }
