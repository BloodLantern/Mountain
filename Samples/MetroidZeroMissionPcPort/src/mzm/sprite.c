...
u8 gSpritesetSpritesID[MAX_AMOUNT_OF_SPRITE_TYPES];
struct SubSpriteData gSubSpriteData1;
struct SubSpriteData gSubSpriteData2;
u8 gParasiteRelated;
struct SpriteData gCurrentSprite;
u8 gPreviousVerticalCollisionCheck;
u8 gPreviousCollisionCheck;
u8 gIgnoreSamusAndSpriteCollision;
u8 gSpriteDrawOrder[MAX_AMOUNT_OF_SPRITES];
struct BossWork gBossWork;
u8 gSpriteRng;
u8 gDisableDrawingSprites;
    ...
    if (gGameModeSub1 == SUB_GAME_MODE_PLAYING)
    {
        ...
        if (!SpriteUtilCheckStopSpritesPose())
        {
            ...
            for (count = 0; count < MAX_AMOUNT_OF_SPRITES; count++)
            {
                ...
                gCurrentSprite = gSpriteData[count];
                ...
                gSpriteData[count] = gCurrentSprite;
            }
...
void SpriteDraw(struct SpriteData* pSprite, s32 slot)
{
    ...
    renderData.xPosition =  ((f32)SUB_PIXEL_TO_PIXEL(pSprite->xPosition) - SUB_PIXEL_TO_PIXEL(gBg1XPosition)) / SCREEN_SIZE_X;
    renderData.yPosition = -((f32)SUB_PIXEL_TO_PIXEL(pSprite->yPosition) - SUB_PIXEL_TO_PIXEL(gBg1YPosition)) / SCREEN_SIZE_Y;
    
    if (pSprite->status & SPRITE_STATUS_UNKNOWN_80)
        renderData.scaling = Q_8_8_TO_INT(pSprite->oamScaling);
    else
    ...
    // SpriteRendererRender(pSprite->spritesetGfxSlot, pSprite->palette, pSprite->pOam[pSprite->currentAnimationFrame].pFrame, &renderData);
    return;
    ...
    gfxOffset = 0; // pSprite->spritesetGfxSlot * 64;
...
void SpriteLoadGfx(u8 spriteId, u8 row)
{
    ...
    LZ77UncompVRAM(sSpritesGraphicsPointers[spriteId], gVramBuffer + 0x14000 + (row * 0x800));
}
...
void SpriteLoadPAL(u8 spriteId, u8 row, u8 len)
{
    ...
    DMA_SET(3, sSpritesPalettePointers[spriteId], gPalramBuffer + 0x300 + (row * 16 * sizeof(u16)), C_32_2_16(DMA_ENABLE, len * 16));
}
