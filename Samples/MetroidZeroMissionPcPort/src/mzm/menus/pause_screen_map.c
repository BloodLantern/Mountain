...
void PauseScreenDrawIgtAndTanks(u8 param_1, u8 drawTanks)
{
    ...
    if (!drawTanks)
    {
        ...
        DmaTransfer(3, gVramBuffer + 0x17EA0 + value * HALF_BLOCK_SIZE,
        ...
        DmaTransfer(3, gVramBuffer + 0x17C00 + value * HALF_BLOCK_SIZE,
    ...
    else
    {
        ...
        for (i = 3; i >= 0; i--)
        {
            ...
            {
                // Checkmark
                DmaTransfer(3, gVramBuffer + 0x17E00, sPauseScreen_40d79c[i] - HALF_BLOCK_SIZE * 3, HALF_BLOCK_SIZE, 32);
            }
            else
            {
                // No checkmark
...
void PauseScreenInitMapDownload(void)
{
    ...
    {
        BitFill(3, 0x1E0, gVramBuffer + 0xD800, 0x800, 16);
    ...
    else if (PAUSE_SCREEN_DATA.mapDownloadType == 2)
    {
        ...
        {
            BitFill(3, 0x1E0 + i, gVramBuffer + 0xD800 + i * MINIMAP_SIZE * 2, MINIMAP_SIZE * 2, 16);
        }
    ...
    {
        ptr_1 = gVramBuffer + 0xE000;
        ptr_2 = gVramBuffer + 0xD800;
        ...
        DMA_SET(3, 0x2034000, gVramBuffer + 0xE000,
...
u32 PauseScreenMapDownload(void)
{
    ...
    switch (PAUSE_SCREEN_DATA.downloadStage)
    {
                ...
                if (PAUSE_SCREEN_DATA.unk_4F < 21)
                {
                    if (PAUSE_SCREEN_DATA.currentDownloadedLine < MINIMAP_SIZE)
                    {
                        ...
                        DMA_SET(3, &gDecompressedMinimapVisitedTiles[PAUSE_SCREEN_DATA.currentDownloadedLine * MINIMAP_SIZE],
                            gVramBuffer + 0xE000 + PAUSE_SCREEN_DATA.currentDownloadedLine * MINIMAP_SIZE * 2, DMA_ENABLE << 16 | MINIMAP_SIZE);   
                    }
        ...
        case 3:
            ...
            DMA_SET(3, gDecompressedMinimapVisitedTiles, gVramBuffer + 0xE000,
                DMA_ENABLE << 16 | ARRAY_SIZE(gDecompressedMinimapVisitedTiles));
...
void MapScreenTogglehWorldMap(u8 forceOff)
{
    ...
    if (PAUSE_SCREEN_DATA.onWorldMap)
    {
        ...
        DmaTransfer(3, PAUSE_SCREEN_EWRAM.worldMapOverlayTilemap, gVramBuffer + 0xD000,
            sizeof(PAUSE_SCREEN_EWRAM.worldMapOverlayTilemap), 16);

        DmaTransfer(3, sPauseScreen_3fd252, gPalramBuffer + 2, sizeof(sPauseScreen_3fd252), 16);
    ...
    else
    {
        ...
        DmaTransfer(3, &sMinimapTilesPal[1], gPalramBuffer + 2, sizeof(sMinimapTilesPal) - 2, 16);
    ...
    switch (PAUSE_SCREEN_DATA.changingMinimapStage)
    {
        ...
        case 2:
            ...
            DmaTransfer(3, PAUSE_SCREEN_DATA.mapsDataPointer[PAUSE_SCREEN_DATA.currentArea],
                gVramBuffer + 0xE000, sizeof(*PAUSE_SCREEN_DATA.mapsDataPointer), 16);
