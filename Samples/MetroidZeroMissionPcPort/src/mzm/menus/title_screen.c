...
{
    LZ77UncompVRAM(pPageData->tiletablePointer, gVramBuffer + pPageData->tiletablePage * 0x800);
}
...
void TitleScreenUpdateAnimatedPalette(void)
{
    ...
    if (TITLE_SCREEN_DATA.activeAnimatedPalettes & TITLE_SCREEN_ANIMATED_PALETTE_SKY_DECORATIONS)
    {
        ...
        if (pAnim->timer == 0)
        {
            ...
            DmaTransfer(3, &sTitleScreenPal[sTitleScreenSkyDecorationsPaletteRows[pAnim->paletteRow] * 16], gPalramBuffer + 0x100, 0x20, 16);
        }
    }
    ...
    if (TITLE_SCREEN_DATA.activeAnimatedPalettes & TITLE_SCREEN_ANIMATED_PALETTE_TITLE)
    {
        ...
        else
        {
            ...
            if (pAnim->timer == 0)
            {
                ...
                DmaTransfer(3, &sTitleScreenPal[sTitleScreenTitlePaletteRows[pAnim->paletteRow] * 16 + 1], gPalramBuffer + 0x2, 0x1E, 16);
            }
        }
    }
    ...
    if (TITLE_SCREEN_DATA.activeAnimatedPalettes & TITLE_SCREEN_ANIMATED_PALETTE_PROMPT)
    {
        ...
        if (pAnim->unk_4 != 0)
        {
            ...
            DmaTransfer(3, &sTitleScreenPromptPAL[pAnim->paletteRow * 16], gPalramBuffer + 0x1A0, 0x18, 16);
        }
        else
        {
            pAnim->timer--;
            ...
            DmaTransfer(3, &sTitleScreenPromptPAL[sTitleScreenPromptPaletteRows[pAnim->paletteRow] * 16], gPalramBuffer + 0x1A0, 0x18, 16);
        }
    }
}
...
void TitleScreenTransferGroundGraphics(void)
{
    ...
    {
        var_0 *= 0x400;
        DmaTransfer(3, src + var_0, gVramBuffer + 0x4000 + var_0, 0x400, 16);
    }
}
...
void TitleScreenInit(void)
{
    ...
    DmaTransfer(3, sTitleScreenPal, gPalramBuffer, sizeof(sTitleScreenPal), 16);
    DmaTransfer(3, sTitleScreenPal, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), sizeof(sTitleScreenPal), 16);
...
void unk_777d8(u8 param_1)
{
    ...
    i = 0;
    mask = 0xFC00;
    dst = gVramBuffer + 0x178 + bgOffset;
