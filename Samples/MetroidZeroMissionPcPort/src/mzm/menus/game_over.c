...
void GameOverInit(void)
{
    ...
    DmaTransfer(3, sGameOverMenuPal, gPalramBuffer, sizeof(sGameOverMenuPal), 16);
    SET_BACKDROP_COLOR(COLOR_BLACK);
    DMA_SET(3, sFileSelectIconsPal, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), C_32_2_16(DMA_ENABLE, ARRAY_SIZE(sFileSelectIconsPal)));

    LZ77UncompVRAM(sGameOverBackgroundTileTable, gVramBuffer + 0x1800);
    LZ77UncompVRAM(sGameOverTextTileTable, gVramBuffer + 0x800);
    LZ77UncompVRAM(sGameOver_454520, gVramBuffer);
    DmaTransfer(3, gVramBuffer + 0x1800, gVramBuffer + 0x1000, 0x800, 32);
    LZ77UncompVRAM(sGameOverTextAndBackgroundGfx, gVramBuffer + 0x4000);
    LZ77UncompVRAM(sFileSelectIconsGfx, VRAM_OBJ);
    LZ77UncompVRAM(sGameOverTextPromptGfxPointers[gLanguage], gVramBuffer + 0xA800);
...
void GameOverUpdateLettersPalette(void)
{
    ...
    GAME_OVER_DATA.dynamicPalette.timer = 0;
    DmaTransfer(3, GAME_OVER_DATA.dynamicPalette.palette, gPalramBuffer + 8, sizeof(GAME_OVER_DATA.dynamicPalette.palette), 16);
}
