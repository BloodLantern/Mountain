...
void EraseSramInit(void)
{
    ...
    DMA_SET(3, sEraseSramMenuBackgroundPal, gPalramBuffer, C_32_2_16(DMA_ENABLE, 0xD0));
    DMA_SET(3, sEraseSramMenuObjectsPal, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), C_32_2_16(DMA_ENABLE, sizeof(sEraseSramMenuObjectsPal) / 2));
    SET_BACKDROP_COLOR(COLOR_BLACK);

    LZ77UncompVRAM(sEraseSramMenuFirstBoxGfx, gVramBuffer + 0x1000);
    LZ77UncompVRAM(sEraseSramMenuObjectsGfx, gVramBuffer + 0x12000);
    LZ77UncompVRAM(sTitleScreenSpaceBackgroundDecorationGfx, gVramBuffer + 0x1800);
    LZ77UncompVRAM(sTitleScreenSpaceAndGroundBackgroundGfx, gVramBuffer + 0x3400);

    LZ77UncompVRAM(sEraseSramTextGfxPointers[ERASE_SRAM_DATA.language][0], gVramBuffer);
    LZ77UncompVRAM(sEraseSramTextGfxPointers[ERASE_SRAM_DATA.language][1], gVramBuffer + 0x11000);
    LZ77UncompVRAM(sEraseSramTextGfxPointers[ERASE_SRAM_DATA.language][0], VRAM_OBJ);

    LZ77UncompVRAM(sEraseSramMenuBoxTileTable, gVramBuffer + 0xD000);
    LZ77UncompVRAM(sEraseSramMenuBackgroundTileTable, gVramBuffer + 0xF000);
