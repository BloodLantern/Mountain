...
u32 PauseScreenUpdateOrStartFading(u8 stage)
{
    ...
    switch (PAUSE_SCREEN_DATA.mapScreenFading.stage)
    {
        ...
        case PAUSE_SCREEN_FADING_IN_INIT:
            DmaTransfer(3, gPalramBuffer, PAUSE_SCREEN_EWRAM.unk_6800, PALRAM_SIZE, 16);
            BitFill(3, 0, gPalramBuffer, PALRAM_SIZE, 16);
            DmaTransfer(3, gPalramBuffer, PAUSE_SCREEN_EWRAM.backgroundPalette, PALRAM_SIZE, 16);
    ...
    if (PAUSE_SCREEN_DATA.mapScreenFading.unk_2)
    {
        DmaTransfer(3, PAUSE_SCREEN_EWRAM.backgroundPalette, gPalramBuffer, PALRAM_SIZE, 16);
        PAUSE_SCREEN_DATA.mapScreenFading.unk_2 = FALSE;
    }
...
void PauseScreenInit(void)
{
    ...
    {
        DmaTransfer(3, VRAM_OBJ, gEwramBuffer + 0x1E000, 0x8000, 16);
    }
    ...
    DmaTransfer(3, sTankIconsPal, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), sizeof(sTankIconsPal), 16);
    DmaTransfer(3, sMinimapTilesPal, gPalramBuffer, sizeof(sMinimapTilesPal), 16);
    DmaTransfer(3, sPauseScreen_3fcef0, gPalramBuffer + 0xA0, sizeof(sPauseScreen_3fcef0), 16);
    sBgPalramPointer[0] = 0;

    DmaTransfer(3, sMinimapTilesGfx, gVramBuffer + 0x8000, 0x3000, 16);
    DmaTransfer(3, gVramBuffer + 0xA820, gVramBuffer + 0xAC20, 0x3E0, 32);


    CallLZ77UncompVram(sTankIconsGfx, gVramBuffer + 0x13000);
    CallLZ77UncompVram(sMapScreenAreaNamesGfxPointers[gLanguage], gVramBuffer + 0x10800);
    CallLZ77UncompVram(sMapScreenUnknownItemsNamesGfxPointers[gLanguage], gVramBuffer + 0x11400);
    CallLZ77UncompVram(sMapScreenChozoStatueAreaNamesGfxPointers[gLanguage], gVramBuffer + 0x11800);

    if (PAUSE_SCREEN_DATA.typeFlags & PAUSE_SCREEN_TYPE_CHOZO_STATUE_HINT)
    {
        CallLZ77UncompVram(sChozoHintBackgroundGfx, gVramBuffer);
        CallLZ77UncompVram(sMapScreenAreaNamesGfx, gVramBuffer + 0xA800);
    }
    else
    {
        CallLZ77UncompVram(sMotifBehindWireframeSamusGfx, gVramBuffer);
        CallLZ77UncompVram(sPauseScreenHudGfx, gVramBuffer + 0x1000);
        CallLZ77UncompVram(sMinimapLettersGfx, gVramBuffer + 0x7400);
        CallLZ77UncompVram(sMapScreenEquipmentNamesGfxPointers[gLanguage], gVramBuffer + 0x6000);
        CallLZ77UncompVram(sMapScreenMenuNamesGfxPointers[gLanguage], gVramBuffer + 0xC00);
        DmaTransfer(3, gVramBuffer + 0x6000, VRAM_OBJ, 0x800, 32);
    }


    BitFill(3, 0x1140, gVramBuffer + 0xE800, 0x1800, 16);
    DmaTransfer(3, gDecompressedMinimapVisitedTiles, gVramBuffer + 0xE000, sizeof(gDecompressedMinimapVisitedTiles), 16);

    if (PAUSE_SCREEN_DATA.typeFlags & PAUSE_SCREEN_TYPE_CHOZO_STATUE_HINT)
    {
        CallLZ77UncompVram(sChozoHintBackgroundTileTable, gVramBuffer + 0xB800);
        BitFill(3, 0x115F, gVramBuffer + 0xC000, 0x800, 16);
    }
    else
    {
        CallLZ77UncompWram(sMapScreenVisorOverlayTilemap, PAUSE_SCREEN_EWRAM.visorOverlayTilemap);
        DmaTransfer(3, PAUSE_SCREEN_EWRAM.visorOverlayTilemap, gVramBuffer + 0xC800,
        ...
        else
        {
            ...
            {
                CallLZ77UncompVram(sDebugMenuTileParts, gVramBuffer + 0xB000);
            ...
            if (gEquipment.suitType == SUIT_SUITLESS)
            {
                DmaTransfer(3, PAUSE_SCREEN_EWRAM.statusScreenBackgroundTilemap, gVramBuffer + 0xC000, 0x500, 16);
            }
            else
            {
                DmaTransfer(3, PAUSE_SCREEN_EWRAM.unk_7800, gVramBuffer + 0xC000, 0x500, 16);
            }
            ...
            CallLZ77UncompVram(sMapScreenTextBg0TileTable, gVramBuffer + 0xD800);
    ...
    else
    {
        ...
        DmaTransfer(3, PAUSE_SCREEN_EWRAM.mapScreenOverlayTilemap, gVramBuffer + 0xD000,
...
void PauseScreenUpdateTopVisorOverlay(u8 oamId)
{
    ...
    DmaTransfer(3, &PAUSE_SCREEN_EWRAM.visorOverlayTilemap[0x280 + offset * 0x40], gVramBuffer + 0xC800, 0x80, 16);
}
...
s32 PauseScreenSuitChangingStart(void)
{
    ...
    switch (PAUSE_SCREEN_DATA.subroutineInfo.stage)
    {
        ...
        case 3:
            ...
            BitFill(3, 0, gVramBuffer + 0x7800, 0x800, 16);
...
s32 PauseScreenStatusScreenInit(void)
{
    ...
    switch (PAUSE_SCREEN_DATA.subroutineInfo.stage)
    {
        case 0:
            ...
            if (gEquipment.suitType == SUIT_SUITLESS)
            {
                DmaTransfer(3, PAUSE_SCREEN_EWRAM.statusScreenBackgroundTilemap, gVramBuffer + 0xC000, 0x500, 32);
            }
            else
            {
                DmaTransfer(3, PAUSE_SCREEN_EWRAM.unk_7800, gVramBuffer + 0xC000, 0x500, 32);
            }
...
s32 PauseScreenEasySleepInit(void)
{
    ...
    switch (PAUSE_SCREEN_DATA.subroutineInfo.stage)
    {
        ...
        case 2:
            // Transfer tilemap
            DmaTransfer(3, PAUSE_SCREEN_EWRAM.easySleepTilemap, gVramBuffer + 0xD000, sizeof(PAUSE_SCREEN_EWRAM.easySleepTilemap), 16);
            break;

        case 3:
            // Make a backup of some Gfx
            DmaTransfer(3, gVramBuffer + 0x6000, PAUSE_SCREEN_EWRAM.equipmentNamesGfxBackup,
                sizeof(PAUSE_SCREEN_EWRAM.equipmentNamesGfxBackup), 16);
            DmaTransfer(3, gVramBuffer + 0x6800, PAUSE_SCREEN_EWRAM.unk_b000,
                sizeof(PAUSE_SCREEN_EWRAM.unk_b000), 16);
            break;

        case 4:
            // More backups
            DmaTransfer(3, gVramBuffer + 0x7000, PAUSE_SCREEN_EWRAM.unk_b800,
                sizeof(PAUSE_SCREEN_EWRAM.unk_b800), 16);
            DmaTransfer(3, gVramBuffer + 0x7800, PAUSE_SCREEN_EWRAM.unk_c000,
        ...
        case 7:
            ...
            {
                DmaTransfer(3, PAUSE_SCREEN_EWRAM.easySleepTextFormatted_1, gVramBuffer + 0x6000,
                    sizeof(PAUSE_SCREEN_EWRAM.easySleepTextFormatted_1), 16);

                DmaTransfer(3, PAUSE_SCREEN_EWRAM.easySleepTextFormatted_2, gVramBuffer + 0x6800,
        ...
        case 8:
            ...
            if (PAUSE_SCREEN_DATA.easySleepTextState < 0)
            {
                DmaTransfer(3, PAUSE_SCREEN_EWRAM.unk_5000, gVramBuffer + 0x7000,
                    sizeof(PAUSE_SCREEN_EWRAM.unk_5000), 16);

                DmaTransfer(3, PAUSE_SCREEN_EWRAM.unk_5800, gVramBuffer + 0x7800,
...
s32 PauseScreenQuitEasySleep(void)
{
    switch (PAUSE_SCREEN_DATA.subroutineInfo.stage)
    {
        ...
        case 3:
            DMA_SET(3, PAUSE_SCREEN_EWRAM.mapScreenOverlayTilemap, gVramBuffer + 0xD000,
                DMA_ENABLE << 16 | ARRAY_SIZE(PAUSE_SCREEN_EWRAM.mapScreenOverlayTilemap));
            break;

        case 4:
            DmaTransfer(3, PAUSE_SCREEN_EWRAM.equipmentNamesGfxBackup, gVramBuffer + 0x6000,
                sizeof(PAUSE_SCREEN_EWRAM.equipmentNamesGfxBackup), 16);
            DmaTransfer(3, PAUSE_SCREEN_EWRAM.unk_b000, gVramBuffer + 0x6800,
                sizeof(PAUSE_SCREEN_EWRAM.unk_b000), 16);
            break;

        case 5:
            DmaTransfer(3, PAUSE_SCREEN_EWRAM.unk_b800, gVramBuffer + 0x7000,
                sizeof(PAUSE_SCREEN_EWRAM.unk_b800), 16);
            DmaTransfer(3, PAUSE_SCREEN_EWRAM.unk_c000, gVramBuffer + 0x7800,
                sizeof(PAUSE_SCREEN_EWRAM.unk_c000), 16);
            break;
