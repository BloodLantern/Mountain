...
struct ColorFading gColorFading;
u16 gBg1CntDuringDoorTransition;
u16 gBg3CntDuringDoorTransition;
...
void unk_5b24c(void)
{
    DmaTransfer(3, gPalramBuffer, gEwramBuffer + 0x35400, PALRAM_SIZE / 2, 16);
    DmaTransfer(3, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), gEwramBuffer + 0x35600, PALRAM_SIZE / 2, 16);
}
...
void unk_5b288(void)
{
    ...
    DmaTransfer(3, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), gEwramBuffer + 0x35200, PALRAM_SIZE / 2, 16);
}
...
void unk_5b2c4(void)
{
    DmaTransfer(3, gEwramBuffer + 0x35400, gEwramBuffer + 0x35000, PALRAM_SIZE / 2, 16);
    DmaTransfer(3, gEwramBuffer + 0x35600, gEwramBuffer + 0x35200, PALRAM_SIZE / 2, 16);
}
...
void unk_5b304(void)
{
    DmaTransfer(3, gPalramBuffer, gEwramBuffer + 0x35800, PALRAM_SIZE / 2, 16);
    DmaTransfer(3, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), gEwramBuffer + 0x35A00, PALRAM_SIZE / 2, 16);
}
...
void unk_5b350(void)
{
    DmaTransfer(3, gEwramBuffer + 0x35400, gEwramBuffer + 0x35800, PALRAM_SIZE / 2, 16);
    DmaTransfer(3, gEwramBuffer + 0x35600, gEwramBuffer + 0x35A00, PALRAM_SIZE / 2, 16);
}
...
void CheckTransferFadedPalette_Unused(void)
{
    ...
    {
        DmaTransfer(3, gEwramBuffer + 0x35000, gPalramBuffer, PALRAM_SIZE / 2, 16);
        gColorFading.status &= ~COLOR_FADING_STATUS_ON_BG;
    }

    if (gColorFading.status & COLOR_FADING_STATUS_ON_OBJ)
    {
        DmaTransfer(3, gEwramBuffer + 0x35200, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]), PALRAM_SIZE / 2, 16);
...
void CheckTransferFadedPalette(void)
{
    if (gGameModeSub1 == SUB_GAME_MODE_PLAYING)
    {
        ...
        if (gColorFading.status & COLOR_FADING_STATUS_ON_OBJ)
        {
            DmaTransfer(3, gEwramBuffer + 0x35200 + 16 * 4, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]) + 16 * 4, PALRAM_SIZE / 2 - 16 * 4, 16);
            gColorFading.status ^= COLOR_FADING_STATUS_ON_OBJ;
        }
    }
    else
...
void PowerBombYellowTint(u8 paletteRow)
{
    ...
    pPalette = (u16*)(gEwramBuffer + 0x35000) + paletteRow * 16;
...
void ApplySpecialBackgroundEffectColorOnBG(u16 mask, u16 color, u8 stage)
{
    ...
    if (stage == 0)
    {
        DmaTransfer(3, gEwramBuffer + 0x35800, gPalramBuffer, 0x1E0, 16);
        DmaTransfer(3, gEwramBuffer + 0x35800, gEwramBuffer + 0x35400, 0x1E0, 16);
        return;
    }

    src = gEwramBuffer + 0x35800;
    dst = gEwramBuffer + 0x35000;
...
void ApplySpecialBackgroundEffectColorOnOBJ(u16 mask, u16 color, u8 stage)
{
    ...
    if (stage == 0)
    {
        DmaTransfer(3, gEwramBuffer + 0x35A40, (&gPalramBuffer[sizeof(gPalramBuffer) / 2]) + 16 * 4, 0x1C0, 16);
        DmaTransfer(3, gEwramBuffer + 0x35A40, gEwramBuffer + 0x35640, 0x1C0, 16);
        return;
    }

    DmaTransfer(3, gEwramBuffer + 0x35A40, gEwramBuffer + 0x35240, 0x1C0, 16);
