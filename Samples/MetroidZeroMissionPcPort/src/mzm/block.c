...
struct BombChain gBombChains[MAX_AMOUNT_OF_BOMB_CHAINS];
u8 gActiveBombChainTypes;
...
void BlockShiftNeverReformBlocks(void)
{
    ...
    src = gNeverReformBlocks[gAreaBeforeTransition];
    ...
    while (amount < gNumberOfNeverReformBlocks[gAreaBeforeTransition] * 2)
    {
        ...
        {
            dst = gEwramBuffer + gNumberOfNeverReformBlocks[gAreaBeforeTransition] * 2;
            while (*dst != UCHAR_MAX)
            {
                src[i++] = *dst++;
                src[i++] = *dst++;
...
void BlockUpdateBrokenBlockAnimation(struct BrokenBlock* pBlock)
{
    ...
    // Apply to tilemap
    dst = gVramBuffer + 0x1000;
    if (pBlock->xPosition & 0x10)
        dst = gVramBuffer + 0x1800;
