...
{
    // Clear Ewram buffer
    BitFill(3, USHORT_MAX, &gSram, sizeof(struct Sram), 16);

    if (!gSramCorruptFlag)
    {
        // Send cleared Ewram buffer to flash
        SramWriteChecked((u8*)&gSram, gSramBuffer, sizeof(struct Sram));
    }
}
        ...
        switch (operation)
        {
            ...
            case 12:
                diff = SramWriteChecked((u8*)&gSram.unk_7f70,
                    gSramBuffer + OFFSET_OF(struct Sram, unk_7f70), sizeof(gSram.unk_7f70));
                break;

            case SRAM_OPERATION_SAVE_DEMO_RAM:
                diff = SramWriteChecked((u8*)&gSram.demoSave,
                    gSramBuffer + OFFSET_OF(struct Sram, demoSave), sizeof(struct SaveDemo));
                break;

            case SRAM_OPERATION_READ_ALL_FLASH:
                SramWriteUnchecked(gSramBuffer, (u8*)&gSram, sizeof(struct Sram));
                break;
            ...
            case SRAM_OPERATION_CHECK_ALL:
                diff = SramCheck((u8*)sSramFlashPointer, (u8*)sSramEwramPointer, sizeof(struct Sram));
                break;
        }
    }
...
u32 unk_fbc(u8 param_1)
{
    ...
    FILE* f;
    fopen_s(&f, "save.bin", "wb");

    if (f)
    {
        fwrite(&gSramBuffer, sizeof(gSramBuffer), 1, f);
