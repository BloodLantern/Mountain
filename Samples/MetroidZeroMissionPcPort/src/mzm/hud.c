...
u8 gPowerBombHighlightStatus;
u8 gSuperMissileHighlightStatus;

struct HudDigits gEnergyDigits;
struct HudDigits gMaxEnergyDigits;
struct HudDigits gMissileDigits;
struct HudDigits gPowerBombDigits;
struct HudDigits gSuperMissileDigits;

u8 gEnergyRefillAnimation;
u8 gMissileRefillAnimation;
u8 gSuperMissileRefillAnimation;
u8 gPowerBombRefillAnimation;

u8 gAmmoDigitsGfx[64];

struct SpriteGraphics* gCommonSpriteGraphics;
struct SpritePalette* gCommonSpritePalette;

static const float sHudOam_HealthBar[] = {
    0
};
...
void HudDrawEnergy(u8 fileNumber)
{
    ...
    if (fileNumber != 0)
    {
        if (fileNumber < 4)
        {
            ...
            dst = (gVramBuffer + 0x20 + (fileNumber - 1) * 0xE0);
        }
        else
            return;
    }
    else
        dst = (gVramBuffer + 0x10A00);
...
void HudDrawMissiles(u8 updateHighlight)
{
    ...
    if (needUpdate)
    {
        ...
        DMA_SET(3, gAmmoDigitsGfx, gVramBuffer + 0x10E40, DMA_ENABLE << 16 | 32);
    }
    ...
    if (gMissileRefillAnimation != 0x0 && !updateHighlight)
    {
        ...
        {
            missiles = 0x480;
            DMA_SET(3, sMissileHUDGfx_Refill1, gVramBuffer + 0x10E00, DMA_ENABLE << 16 | 32);
        }
        else if (gMissileRefillAnimation == 0xA)
        {
            missiles = 0x600;
            DMA_SET(3, sMissileHUDGfx_Refill2, gVramBuffer + 0x10E00, DMA_ENABLE << 16 | 32);
        }
        else if (gMissileRefillAnimation == 0x4)
        {
            missiles = 0x780;
            DMA_SET(3, sMissileHUDGfx_Refill3, gVramBuffer + 0x10E00, DMA_ENABLE << 16 | 32);
        }
        else if (gMissileRefillAnimation == 0x1)
        {
            needUpdate = gMissileHighlightStatus;
            missiles = needUpdate * 384;
            DMA_SET(3, sMissileHUDGfx_Inactive + (gMissileHighlightStatus * 64), gVramBuffer + 0x10E00, DMA_ENABLE << 16 | 32);
        }
        else
            return;
...
void HudDrawPowerBomb(u8 updateHighlight)
{
    ...
    if (needUpdate)
    {
        ...
        DMA_SET(3, gAmmoDigitsGfx, gVramBuffer + 0x10B40, DMA_ENABLE << 16 | 32);
    }
    ...
    if (gPowerBombRefillAnimation != 0x0 && !updateHighlight)
    {
        ...
        {
            powerBombs = 0x480;
            DMA_SET(3, sPowerBombHUDGfx_Refill1, gVramBuffer + 0x10B00, DMA_ENABLE << 16 | 32);
        }
        else if (gPowerBombRefillAnimation == 0xA)
        {
            powerBombs = 0x600;
            DMA_SET(3, sPowerBombHUDGfx_Refill2, gVramBuffer + 0x10B00, DMA_ENABLE << 16 | 32);
        }
        else if (gPowerBombRefillAnimation == 0x4)
        {
            powerBombs = 0x780;
            DMA_SET(3, sPowerBombHUDGfx_Refill3, gVramBuffer + 0x10B00, DMA_ENABLE << 16 | 32);
        }
        else if (gPowerBombRefillAnimation == 0x1)
        {
            needUpdate = gPowerBombHighlightStatus;
            powerBombs = needUpdate * 384;
            DMA_SET(3, sPowerBombHUDGfx_Inactive + (gPowerBombHighlightStatus * 64), gVramBuffer + 0x10B00, DMA_ENABLE << 16 | 32);
        }
        else
            return;
...
void HudDrawSuperMissiles(u8 updateHighlight)
{
    ...
    if (needUpdate)
    {
        ...
        DMA_SET(3, gAmmoDigitsGfx, gVramBuffer + 0x10EC0, DMA_ENABLE << 16 | 32);
    }
    ...
    if (gSuperMissileRefillAnimation != 0x0 && !updateHighlight)
    {
        ...
        {
            superMissiles = 0x480;
            DMA_SET(3, sSuperMissileHUDGfx_Refill1, gVramBuffer + 0x10E80, DMA_ENABLE << 16 | 32);
        }
        else if (gSuperMissileRefillAnimation == 0xA)
        {
            superMissiles = 0x600;
            DMA_SET(3, sSuperMissileHUDGfx_Refill2, gVramBuffer + 0x10E80, DMA_ENABLE << 16 | 32);
        }
        else if (gSuperMissileRefillAnimation == 0x4)
        {
            superMissiles = 0x780;
            DMA_SET(3, sSuperMissileHUDGfx_Refill3, gVramBuffer + 0x10E80, DMA_ENABLE << 16 | 32);
        }
        else if (gSuperMissileRefillAnimation == 0x1)
        {
            needUpdate = gSuperMissileHighlightStatus;
            superMissiles = needUpdate * 384;
            DMA_SET(3, sSuperMissileHUDGfx_Inactive + (gSuperMissileHighlightStatus * 64), gVramBuffer + 0x10E80, DMA_ENABLE << 16 | 32);
        }
        else
            return;
...
void HUDUpdateGfx(void)
{
    ...
    if (gMissileHighlightStatus == HIGHLIGHT_STATUS_HIGHLIGHTED)
    {
        if (!(gSamusWeaponInfo.weaponHighlighted & WH_MISSILE))
        {
            ...
            {
                DMA_SET(3, sMissileHUDGfx_Inactive, gVramBuffer + 0x10E00, DMA_ENABLE << 16 | 32);
                gMissileHighlightStatus = HIGHLIGHT_STATUS_NOT_HIGHLIGHTED;
            }
            else
            {
                DMA_SET(3, sMissileHUDGfx_Selected, gVramBuffer + 0x10E00, DMA_ENABLE << 16 | 32);
    ...
    else if (gMissileHighlightStatus == HIGHLIGHT_STATUS_SELECTED)
    {
        ...
        {
            DMA_SET(3, sMissileHUDGfx_Active, gVramBuffer + 0x10E00, DMA_ENABLE << 16 | 32);
    ...
    if (gPowerBombHighlightStatus == HIGHLIGHT_STATUS_HIGHLIGHTED)
    {
        if (!(gSamusWeaponInfo.weaponHighlighted & WH_POWER_BOMB))
        {
            ...
            {
                DMA_SET(3, sPowerBombHUDGfx_Selected, gVramBuffer + 0x10B00, DMA_ENABLE << 16 | 32);
                gPowerBombHighlightStatus = HIGHLIGHT_STATUS_SELECTED;
            }
            else
            {
                DMA_SET(3, sPowerBombHUDGfx_Inactive, gVramBuffer + 0x10B00, DMA_ENABLE << 16 | 32);
    ...
    if (gPowerBombHighlightStatus == HIGHLIGHT_STATUS_SELECTED)
    {
        ...
        {
            DMA_SET(3, sPowerBombHUDGfx_Active, gVramBuffer + 0x10B00, DMA_ENABLE << 16 | 32);
    ...
    if (gSuperMissileHighlightStatus == HIGHLIGHT_STATUS_HIGHLIGHTED)
    {
        if (!(gSamusWeaponInfo.weaponHighlighted & WH_SUPER_MISSILE))
        {
            ...
            {
                DMA_SET(3, sSuperMissileHUDGfx_Inactive, gVramBuffer + 0x10E80, DMA_ENABLE << 16 | 32);
                gSuperMissileHighlightStatus = HIGHLIGHT_STATUS_NOT_HIGHLIGHTED;
            }
            else
            {
                DMA_SET(3, sSuperMissileHUDGfx_Selected, gVramBuffer + 0x10E80, DMA_ENABLE << 16 | 32);
    ...
    else if (gSuperMissileHighlightStatus == HIGHLIGHT_STATUS_SELECTED)
    {
        ...
        {
            DMA_SET(3, sSuperMissileHUDGfx_Active, gVramBuffer + 0x10E80, DMA_ENABLE << 16 | 32);
    ...
    {
        gUpdateMinimapFlag--;
        DMA_SET(3, (gMinimapTilesGfx + gUpdateMinimapFlag * 24), (gVramBuffer + 0x11F80) + (gUpdateMinimapFlag * 1024), DMA_ENABLE << 16 | 48);
    }
}
...
void HudDrawSuitless(void)
{
    ...
    {
        DMA_SET(3, &sSuitlessHUDChargeBarGfx[chargeCounter * SUITLESS_BAR_SIZE], gVramBuffer + 0x11100,
