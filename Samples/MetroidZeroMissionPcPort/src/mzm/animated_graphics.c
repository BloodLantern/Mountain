...
struct BackgroundEffect gBackgroundEffect;
struct AnimatedTiming gTankAnimations[4];
struct AnimatedGraphicsInfo gAnimatedGraphicsData[16];
struct AnimatedPaletteTiming gAnimatedPaletteTiming;

s8 gDisableAnimatedPalette;
u8 gDisableAnimatedGraphicsTimer;
u16 gAnimatedGraphicsToUpdate;
...
u32 BackgroundEffectStart(u8 effect)
{
    if (gBackgroundEffect.type == 0 && effect != UCHAR_MAX)
    {
        ...
        DmaTransfer(3, gEwramBuffer + 0x35400, gEwramBuffer + 0x35800, 0x200, 16);
        DmaTransfer(3, gEwramBuffer + 0x35600, gEwramBuffer + 0x35A00, 0x200, 16);
        return TRUE;
    }
