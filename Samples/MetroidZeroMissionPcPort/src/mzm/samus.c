...
struct SamusData gSamusDataCopy;
struct WeaponInfo gSamusWeaponInfo;
struct SamusEcho gSamusEcho;
struct ScrewSpeedAnimation gScrewSpeedAnimation;
struct Equipment gEquipment;
struct HazardDamage gSamusHazardDamage;
struct EnvironmentalEffect gSamusEnvironmentalEffects[5];
struct SamusPhysics gSamusPhysics;
u16 gPreviousXPosition;
u16 gPreviousYPosition;
u16 gPreventMovementTimer;
u8 gDisableDrawingSamusAndScrolling;

u16 gSamusPalette[16 * 2];
u16 gSamusPaletteSize;
s16 gSamusDoorPositionOffset;
...
void SamusInit(void)
{
    ...
    if (gGameModeSub3 == 0)
    {
        if (!gIsLoadingFile)
        {
            ...
            memset(&gSamusData, 0, sizeof(gSamusData));
            memset(&gEquipment, 0, sizeof(gEquipment));
            memset(&gSamusWeaponInfo, 0, sizeof(gSamusWeaponInfo));
            memset(&gScrewSpeedAnimation, 0, sizeof(gScrewSpeedAnimation));
        ...
        {
            // Clear physics
            memset(&gSamusPhysics, 0, sizeof(gSamusPhysics));
        }
    }
}
