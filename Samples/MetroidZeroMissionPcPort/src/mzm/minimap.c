...
u32 gVisitedMinimapTiles[MAX_AMOUNT_OF_AREAS * MINIMAP_SIZE];
u16 gDecompressedMinimapVisitedTiles[MINIMAP_SIZE * MINIMAP_SIZE];
u16 gDecompressedMinimapData[MINIMAP_SIZE * MINIMAP_SIZE];
u8 gMinimapX;
u8 gMinimapY;
u8 gUpdateMinimapFlag;
u32 gMinimapTilesGfx[5 * 24];
...
void MinimapUpdateForExploredTiles(void)
{
    ...
    tiles = gDecompressedMinimapVisitedTiles + offset; // gDecompressedMinimapVisitedTiles
...
void MinimapDraw(void)
{
    ...
    dst = gMinimapTilesGfx + (gUpdateMinimapFlag - 1) * 24;
...
void MinimapSetTilesWithObtainedItems(u8 area, u16* dst)
{
    ...
    src = (u32*)(gMinimapTilesWithObtainedItems + area * 512);
