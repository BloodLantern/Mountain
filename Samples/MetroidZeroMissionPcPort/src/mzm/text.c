...
void TextDrawlocation(u8 locationText, u8 gfxSlot)
{
    ...
    BitFill(3, 0xFFFF, gEwramBuffer, 0x800, 0x10);

    pText = sLocationTextPointers[gLanguage][locationText];
    TextDrawLocationTextCharacters(1, &pText);

    DMA_SET(3, gEwramBuffer, gVramBuffer + 0x14000 + gfxSlot * 0x800, (DMA_ENABLE | DMA_32BIT) << 16 | 0xE0);
    DMA_SET(3, gEwramBuffer + 0x400, gVramBuffer + 0x14400 + gfxSlot * 0x800, (DMA_ENABLE | DMA_32BIT) << 16 | 0xE0);
}
...
u8 unk_6f0a8(u8 textID, u8 gfxSlot, u8 param_3)
{
    ...
    {
        case 0:
            BitFill(3, -1, gVramBuffer + 0x14000 + gCurrentMessage.gfxSlot * 0x800, 0x380, 32);
            BitFill(3, -1, gVramBuffer + 0x14400 + gCurrentMessage.gfxSlot * 0x800, 0x380, 32);
            gCurrentMessage.stage++;
            break;

        case 1:
            BitFill(3, -1, gVramBuffer + 0x14800 + gCurrentMessage.gfxSlot * 0x800, 0x380, 32);
            BitFill(3, -1, gVramBuffer + 0x14C00 + gCurrentMessage.gfxSlot * 0x800, 0x380, 32);
            gCurrentMessage.stage++;
            break;
        ...
        case 2:
            ...
            for (; i != 0; i--)
            {
                switch (TextProcessCurrentMessage(&gCurrentMessage, sMessageTextPointers[gLanguage][gCurrentMessage.messageID],
                    gVramBuffer + 0x14000 + gCurrentMessage.gfxSlot * 0x800 + gCurrentMessage.line * 0x800))
                {
...
u8 TextProcessStory(void)
{
    ...
    switch (gCurrentMessage.stage)
    {
        case 0:
            ...
            {
                BitFill(3, 0, gVramBuffer + 0x7000, 0x380, 0x20);
                BitFill(3, 0, gVramBuffer + 0x7400, 0x380, 0x20);
                gCurrentMessage.stage = 1;
            }
            break;

        case 1:
            BitFill(3, 0, gVramBuffer + 0x7800, 0x380, 0x20);
            BitFill(3, 0, gVramBuffer + 0x7C00, 0x380, 0x20);
            gCurrentMessage.stage++;
            break;
        ...
        case 2:
            ...
            if (gCurrentMessage.gfxSlot != 0)
            {
                if (gCurrentMessage.line < 10 && !gCurrentMessage.messageEnded)
                    dst = gVramBuffer + 0x13000 + gCurrentMessage.line * 0x800;
                else
                    i = 0;
            }
            else
            {
                if (gCurrentMessage.line >= 2 || gCurrentMessage.messageEnded)
                    ...
                    dst = gVramBuffer + 0x7000 + gCurrentMessage.line * 0x800;
            }
...
u8 TextProcessFileScreenPopUp(void)
{
    ...
    switch (gCurrentMessage.stage)
    {
        case 0:
            ...
            dst = gVramBuffer + gCurrentMessage.line * 0x800;
            for (; i != 0; i--)
            {
    ...
    switch (PAUSE_SCREEN_DATA.unk_56)
    {
        ...
        case 2:
            ...
            result = TextProcessCurrentMessage(&gCurrentMessage,
                sDescriptionTextPointers[gLanguage][PAUSE_SCREEN_DATA.currentEquipment], gVramBuffer + 0x7800);
