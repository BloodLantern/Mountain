...
export struct MATH_API Quaternion
{
    ...
    ///
    /// This constructor assumes that <code>data</code> is a valid pointer pointing to at least four float_t values.
    ///
    ...
    /// @brief Construct a Quaternion from four @c float_t values.
    ...
    /// @brief Returns the @c x component of this Quaternion.
    [[nodiscard]]
    constexpr float_t X() const noexcept;

    /// @brief Returns the @c y component of this Quaternion.
    [[nodiscard]]
    constexpr float_t Y() const noexcept;

    /// @brief Returns the @c z component of this Quaternion.
    [[nodiscard]]
    constexpr float_t Z() const noexcept;

    /// @brief Returns the @c w component of this Quaternion.
    [[nodiscard]]
    ...
    /// @brief Adds two @c Quaternion together.
    ...
    constexpr friend Quaternion operator-(const Quaternion& a) noexcept;

    /// @brief Subtracts a Quaternion from another one.
    constexpr friend Quaternion operator-(const Quaternion& a, const Quaternion& b) noexcept;

    /// @brief Multiplies two @c Quaternion together.
    constexpr friend Quaternion operator*(const Quaternion& a, const Quaternion& b) noexcept;

    /// @brief Multiplies a Quaternion with a Vector3.
    constexpr friend Quaternion operator*(const Quaternion& q, const Vector3& v) noexcept;

    /// @brief Multiplies a Quaternion by a @p factor.
    constexpr friend Quaternion operator*(const Quaternion& q, float_t factor) noexcept;

    /// @brief Multiplies a Quaternion by a @p factor.
    constexpr friend Quaternion operator*(float_t factor, Quaternion q) noexcept;

    /// @brief Divides a Quaternion by a @p factor.
    constexpr friend Quaternion operator/(const Quaternion& v, float_t factor) noexcept;

    /// @brief Adds two @c Quaternion according to @c operator+(const Quaternion&, const Quaternion&), placing the result in @p a.
    constexpr friend Quaternion& operator+=(Quaternion& a, const Quaternion& b) noexcept;

    /// @brief Subtracts two @c Quaternion according to @c operator-(const Quaternion&, const Quaternion&), placing the result in @p a.
    constexpr friend Quaternion& operator-=(Quaternion& a, const Quaternion& b) noexcept;

    /// @brief Multiplies two @c Quaternion according to @c operator*(const Quaternion&, const Quaternion&), placing the result in @p a.
    constexpr friend Quaternion& operator*=(Quaternion& a, const Quaternion& b) noexcept;

    /// @brief Multiplies a Quaternion by a factor according to @c operator*(const Quaternion&, const float_t), placing the result in @p a.
    constexpr friend Quaternion& operator*=(Quaternion& q, float_t factor) noexcept;

    /// @brief Divides a Quaternion by a factor according to @c operator/(const Quaternion&, const float_t), placing the result in @p a.
    constexpr friend Quaternion& operator/=(Quaternion& q, float_t factor) noexcept;

    /// @brief Checks if two Quaternions are considered equal.
    constexpr friend bool operator==(const Quaternion& a, const Quaternion& b) noexcept;
...
static_assert(std::is_default_constructible_v<Quaternion>, "Struct Quaternion must be default constructible.");
static_assert(std::is_copy_constructible_v<Quaternion>, "Struct Quaternion must be copy constructible.");
static_assert(std::is_move_constructible_v<Quaternion>, "Struct Quaternion must be move constructible.");
static_assert(std::is_copy_assignable_v<Quaternion>, "Struct Quaternion must be copy assignable.");
static_assert(std::is_move_assignable_v<Quaternion>, "Struct Quaternion must be move assignable.");
...
constexpr void Quaternion::Rotate(const Vector3& point, const Quaternion& rotation, Vector3* result) noexcept
{
    *result = (rotation * point * rotation.Conjugate()).imaginary;
}
...
export [[nodiscard]] constexpr Quaternion operator+(const Quaternion& a, const Quaternion& b) noexcept
{
    return Quaternion(a.imaginary + b.imaginary, a.real + b.real);
}

export [[nodiscard]] constexpr Quaternion operator-(const Quaternion& a) noexcept { return Quaternion(-a.imaginary, -a.real); }

export [[nodiscard]] constexpr Quaternion operator-(const Quaternion& a, const Quaternion& b) noexcept { return Quaternion(a + -b); }
...
export [[nodiscard]] constexpr Quaternion operator*(const Quaternion& q, const Vector3& v) noexcept { return q * Quaternion(v, 1.f); }

export [[nodiscard]] constexpr Quaternion operator*(const Quaternion& q, const float_t factor) noexcept
{
    return Quaternion(q.imaginary * factor, q.real * factor);
}

export [[nodiscard]] constexpr Quaternion operator*(const float_t factor, const Quaternion q) noexcept { return q * factor; }

export [[nodiscard]] constexpr Quaternion operator/(const Quaternion& v, const float_t factor) noexcept
{
    return Quaternion(v.imaginary / factor, v.real / factor);
}

export constexpr Quaternion& operator+=(Quaternion& a, const Quaternion& b) noexcept { return a = a + b; }

export constexpr Quaternion& operator-=(Quaternion& a, const Quaternion& b) noexcept { return a = a - b; }

export constexpr Quaternion& operator*=(Quaternion& a, const Quaternion& b) noexcept { return a = a * b; }

export constexpr Quaternion& operator*=(Quaternion& q, const float_t factor) noexcept { return q = q * factor; }

export constexpr Quaternion& operator/=(Quaternion& q, const float_t factor) noexcept { return q = q / factor; }

export [[nodiscard]] constexpr bool operator==(const Quaternion& a, const Quaternion& b) noexcept
{
...
export [[nodiscard]] constexpr bool operator!=(const Quaternion& a, const Quaternion& b) noexcept { return !(a == b); }
