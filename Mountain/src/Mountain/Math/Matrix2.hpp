...
struct MOUNTAIN_API Matrix2
{
    ...
    static void RotationZ(f32 angle, Matrix2* result) noexcept;
    ...
    constexpr explicit Matrix2(f32 defaultValue) noexcept;

    /// @brief Constructs a Matrix2 with its components set to the data pointed by @c data.
    ///
    /// This constructor assumes that @c data is a valid pointer pointing to at least 4 @c f32 values.
    ///
    /// @param data The data where the values for this matrix's components are located.
    constexpr explicit Matrix2(const f32* data) noexcept;
    ...
    /// @brief Constructs a Matrix2 with 16 f32 values.
    constexpr Matrix2(
            f32 m00, f32 m01,
            f32 m10, f32 m11
    ) noexcept;
...
static_assert(std::is_default_constructible_v<Matrix2>, "Class Matrix2 must be default constructible.");
static_assert(std::is_copy_constructible_v<Matrix2>, "Class Matrix2 must be copy constructible.");
static_assert(std::is_move_constructible_v<Matrix2>, "Class Matrix2 must be move constructible.");
static_assert(std::is_copy_assignable_v<Matrix2>, "Class Matrix2 must be copy assignable.");
static_assert(std::is_move_assignable_v<Matrix2>, "Class Matrix2 must be move assignable.");

constexpr Matrix2::Matrix2(const f32 defaultValue) noexcept
    : m00(defaultValue), m10(defaultValue)
    , m01(defaultValue), m11(defaultValue)
{
}

constexpr Matrix2::Matrix2(const f32* const data) noexcept
...
constexpr Matrix2::Matrix2(
    const f32 m00, const f32 m01,
    const f32 m10, const f32 m11
) noexcept
    : m00(m00), m10(m10)
    , m01(m01), m11(m11)
{
...
constexpr Matrix2& operator*=(Matrix2& m, const f32 scalar) noexcept { return m = m * scalar; }

/// @brief Multiplies a Matrix by a Vector2 according to @ref operator*(const Matrix&, const Vector2&), placing the result in @p m.
constexpr Vector2& operator*=(const Matrix2& m, Vector2& v) noexcept { return v = m * v; }
...
constexpr Matrix2 Matrix2::RotationZ(const f32 cos, const f32 sin) noexcept
{
    return Matrix2(
        cos,   -sin,
        sin,    cos
...
constexpr void Matrix2::RotationZ(const f32 cos, const f32 sin, Matrix2* result) noexcept
{
    *result = Matrix2(
        cos,   -sin,
        sin,    cos
...
constexpr const f32* Matrix2::Data() const noexcept { return &m00; }

constexpr f32* Matrix2::Data() noexcept { return &m00; }

constexpr bool Matrix2::IsDiagonal() const noexcept
{
    return Calc::IsZero(m01) && Calc::IsZero(m10);
}

constexpr bool Matrix2::IsIdentity() const noexcept
{
    if (!IsDiagonal())
        return false;
...
constexpr bool Matrix2::IsNull() const noexcept
{
    if (!IsDiagonal())
        return false;
...
constexpr bool Matrix2::IsSymmetric() const noexcept
{
    return Calc::Equals(m01, m10);
}

constexpr bool Matrix2::IsAntisymmetric() const noexcept
{
    return Calc::Equals(m01, -m10);
}
...
constexpr f32 Matrix2::Trace() const noexcept { return m00 + m11; }

constexpr f32 Matrix2::Determinant() const noexcept
{
...
MOUNTAIN_API std::ostream& operator<<(std::ostream& out, const Matrix2& m);
