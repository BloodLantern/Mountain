...
struct MOUNTAIN_API Matrix3
{
    ...
    static void Rotation(f32 angle, const Vector3& axis, Matrix3* result) noexcept;
    ...
    /// @param angle The angle in radians.
    /// @param result The output of the function.
    static void RotationX(f32 angle, Matrix3* result) noexcept;
    ...
    /// @param sin The sine of the angle in radians.
    /// @param result The output of the function.
    static constexpr void RotationX(f32 cos, f32 sin, Matrix3* result) noexcept;
    ...
    /// @param angle The angle in radians.
    /// @param result The output of the function.
    static void RotationY(f32 angle, Matrix3* result) noexcept;
    ...
    /// @param sin The sine of the angle in radians.
    /// @param result The output of the function.
    static constexpr void RotationY(f32 cos, f32 sin, Matrix3* result) noexcept;
    ...
    /// @param angle The angle in radians.
    /// @param result The output of the function.
    static void RotationZ(f32 angle, Matrix3* result) noexcept;
    ...
    constexpr explicit Matrix3(f32 defaultValue) noexcept;

    /// @brief Constructs a Matrix3 with its components set to the data pointed by @c data.
    ///
    /// This constructor assumes that @c data is a valid pointer pointing to at least 9 @c f32 values.
    ///
    /// @param data The data where the values for this matrix's components are located.
    constexpr explicit Matrix3(const f32* data) noexcept;
    ...
    /// @brief Constructs a Matrix3 with 16 f32 values.
    constexpr Matrix3(
...
constexpr Matrix3::Matrix3(const f32 defaultValue) noexcept
...
constexpr Matrix3::Matrix3(const f32* const data) noexcept
...
constexpr Matrix3& operator*=(Matrix3& m, const f32 scalar) noexcept { return m = m * scalar; }

/// @brief Multiplies a Matrix by a Vector2 according to @ref operator*(const Matrix&, const Vector2&), placing the result in @p m.
constexpr Vector2& operator*=(const Matrix3& m, Vector2& v) noexcept { return v = m * v; }
...
constexpr Matrix3 Matrix3::RotationX(const f32 cos, const f32 sin) noexcept
{
    return Matrix3(
        1.f,    0.f,    0.f,
        0.f,    cos,   -sin,
...
constexpr void Matrix3::RotationX(const f32 cos, const f32 sin, Matrix3* result) noexcept
{
    *result = Matrix3(
        1.f,    0.f,    0.f,
        0.f,    cos,   -sin,
...
constexpr Matrix3 Matrix3::RotationY(const f32 cos, const f32 sin) noexcept
{
    return Matrix3(
        cos,    0.f,    sin,
        0.f,    1.f,    0.f,
...
constexpr void Matrix3::RotationY(const f32 cos, const f32 sin, Matrix3* result) noexcept
{
    *result = Matrix3(
        cos,    0.f,    sin,
        0.f,    1.f,    0.f,
...
constexpr Matrix3 Matrix3::RotationZ(const f32 cos, const f32 sin) noexcept
{
    return Matrix3(
        cos,   -sin,    0.f,
        sin,    cos,    0.f,
...
constexpr void Matrix3::RotationZ(const f32 cos, const f32 sin, Matrix3* result) noexcept
{
    *result = Matrix3(
        cos,   -sin,    0.f,
        sin,    cos,    0.f,
...
constexpr const f32* Matrix3::Data() const noexcept { return &m00; }

constexpr f32* Matrix3::Data() noexcept { return &m00; }

constexpr bool Matrix3::IsDiagonal() const noexcept
{
...
constexpr bool Matrix3::IsIdentity() const noexcept
{
    if (!IsDiagonal())
        return false;
...
constexpr bool Matrix3::IsNull() const noexcept
{
    if (!IsDiagonal())
        return false;
...
constexpr bool Matrix3::IsSymmetric() const noexcept
{
    return Calc::Equals(m01, m10) && Calc::Equals(m02, m20)
           && Calc::Equals(m12, m21);
}

constexpr bool Matrix3::IsAntisymmetric() const noexcept
{
    return Calc::Equals(m01, -m10) && Calc::Equals(m02, -m20)
           && Calc::Equals(m12, -m21);
}
...
constexpr f32 Matrix3::Trace() const noexcept { return m00 + m11 + m22; }

constexpr f32 Matrix3::Determinant() const noexcept
{
...
{
    return Vector3(Data() + static_cast<ptrdiff_t>(col) * 3);
}
...
constexpr Vector3& Matrix3::operator[](const size_t col)
{
    ...
    return *static_cast<Vector3*>(static_cast<void*>(Data() + static_cast<ptrdiff_t>(col) * 3));
}
...
MOUNTAIN_API std::ostream& operator<<(std::ostream& out, const Matrix3& m);
