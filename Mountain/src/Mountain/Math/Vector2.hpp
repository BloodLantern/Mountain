...
struct MOUNTAIN_API Vector2
{
    ...
    constexpr explicit Vector2(f32 xy) noexcept;
    ...
    constexpr explicit Vector2(const f32* data) noexcept;
...
constexpr f32 Vector2::Dot(const Vector2 a, const Vector2 b) noexcept { return a.x * b.x + a.y * b.y; }

constexpr f32 Vector2::Cross(const Vector2 a, const Vector2 b) noexcept { return Determinant(a, b); }
...
constexpr f32 Vector2::SquaredLength() const noexcept { return SQ(x) + SQ(y); }
...
/// @brief Multiplies a Vector2 by a @p factor according to @ref operator*(const Vector2, const f32), placing the result in @p v.
...
/// @brief Divides a Vector2 by a @p factor according to @ref operator/(const Vector2, const f32), placing the result in @p v.
constexpr Vector2& operator/=(Vector2& v, const f32 factor) noexcept { return v = v / factor; }
...
MOUNTAIN_API std::ostream& operator<<(std::ostream& out, Vector2 v) noexcept;
...
template <class ParseContext>
constexpr typename ParseContext::iterator std::formatter<Vector2, c8>::parse(ParseContext& ctx)
{
    auto it = ctx.begin();
    if (it == ctx.end())
        return it;
