...
struct MOUNTAIN_API Vector3
{
	...
	constexpr explicit Vector3(f32 xyz) noexcept;
	...
	constexpr explicit Vector3(const f32* data) noexcept;
...
constexpr Vector3::Vector3(const f32 xyz) noexcept : x(xyz), y(xyz), z(xyz) {}

constexpr Vector3::Vector3(const f32* const data) noexcept : x(data[0]), y(data[1]), z(data[2]) {}

constexpr Vector3::Vector3(const f32 x, const f32 y, const f32 z) noexcept : x(x), y(y), z(z) {}
...
constexpr f32 Vector3::Dot(const Vector3& a, const Vector3& b) noexcept { return a.x * b.x + a.y * b.y + a.z * b.z; }
...
constexpr const f32* Vector3::Data() const noexcept { return &x; }

constexpr f32* Vector3::Data() noexcept { return &x; }

constexpr f32 Vector3::SquaredLength() const noexcept { return SQ(x) + SQ(y) + SQ(z); }
...
/// @brief Multiplies a Vector3 by a @p factor according to @ref operator*(const Vector3&, const f32), placing the result in @p v.
...
/// @brief Divides a Vector3 by a @p factor according to @ref operator/(const Vector3&, const f32), placing the result in @p v.
constexpr Vector3& operator/=(Vector3& v, const f32 factor) noexcept { return v = v / factor; }
...
MOUNTAIN_API std::ostream& operator<<(std::ostream& out, const Vector3& v) noexcept;

constexpr Vector3 Vector3::Combine(const Vector3& a, const Vector3& b, const f32 aScale, const f32 bScale) noexcept { return (a * aScale) + (b * bScale); }
...
template <class ParseContext>
constexpr typename ParseContext::iterator std::formatter<Vector3, c8>::parse(ParseContext& ctx)
{
    auto it = ctx.begin();
    if (it == ctx.end())
        return it;
