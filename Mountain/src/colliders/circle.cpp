...
namespace mountain
{
    ...
    {
        Draw::Circle(Position, mRadius, color);
    }

    bool Circle::CheckCollision(const Vector2& point) const
    {
        return (point - Position).SquaredNorm() <= SQ(mRadius);
    }
    ...
    {
        return (circle.Position - Position).SquaredNorm() < SQ(mRadius + circle.mRadius);
    }
    ...
    bool Circle::Intersect(const Vector2& p1, const Vector2& p2) const
    {
        ...
        if (centerProjection >= p1 && centerProjection <= p2)
            return (centerProjection - Position).SquaredNorm() <= SQ(mRadius);
        return false;
    }
}