...
{
    const Vector2& origin = Renderer::sInstance->ScreenOrigin;
    glBegin(GL_POINTS);
    glColor4ub(color.r, color.g, color.b, color.a);
    for (size_t i = 0; i < count; i++)
        glVertex2f(positions[i].x + origin.x, positions[i].y + origin.y);
    glEnd();
}
...
void mountain::Draw::Line(const Vector2 &p1, const Vector2 &p2, const Color color)
{
    ...
    glBegin(GL_LINES);
    glColor4ub(color.r, color.g, color.b, color.a);
    glVertex2f(p1.x + origin.x, p1.y + origin.y);
    glVertex2f(p2.x + origin.x, p2.y + origin.y);
    glEnd();
}
...
void mountain::Draw::Rect(const Vector2& position, const Vector2& size, const Color color)
{
    ...
    glColor4ub(color.r, color.g, color.b, color.a);
    glVertex2f(position.x + origin.x, position.y + origin.y);
    glVertex2f(position.x + size.x + origin.x, position.y + origin.y);
    glVertex2f(position.x + size.x + origin.x, position.y + size.y + origin.y);
    glVertex2f(position.x + origin.x, position.y + size.y + origin.y);
    glEnd();
}
...
{
    const Vector2& origin = Renderer::sInstance->ScreenOrigin;
    CircleInternal(position + origin, radius, color, segments);
}

void mountain::Draw::CircleFilled(const Vector2 &position, const float radius, const Color color, unsigned int segments)
{
    const Vector2& origin = Renderer::sInstance->ScreenOrigin;
    CircleInternal(position + origin, radius, color, segments, GL_POLYGON);
}
